...
public class ChessBoard
{
    ...
    public float CalсulateAdvantage()
    {
        ...
        King myKing = IsWhiteTurn ? GetWhiteKing() : GetBlackKing();
        King opponentKing = IsWhiteTurn ? GetBlackKing() : GetWhiteKing();

        coefficient += GetKingSafetyBonus(myKing);
        opponentCoefficient += GetKingSafetyBonus(opponentKing);
    ...
    private float GetKingSafetyBonus(King king)
    {
        ...
        for (int i = -1; i <= 1; i++)
        {
            ...
            Figure shieldPiece = GetFigureAt(shieldLetter, shieldRank);
